 ...
 */
@Component
public class ExportStudentTools {
...
//            System.out.println("========="+methodName);
    ...
    public void export(String title, String sheetName, List data, String[]columns, String[]fields, OutputStream os) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            for (int j = 0; j < columns.length; j++) {
                ...
                else if(j==4) {cell.setCellValue("1".equals(as.getStatus())?"通过":"驳回");}
                else if(j==5) {cell.setCellValue("1".equals(as.getHasCheck())?"已签到":"未签到");}
                else if(j==6) {cell.setCellValue("             ");}*/
    ...
    public void exportStudent(List<ActivityStudent> data, OutputStream os) {
        String title = "活动人员名单-"+ NormalTools.curDate("yyyyMMdd")+"-（"+data.size()+"人）";
        XSSFWorkbook workbook = new XSSFWorkbook();                        // 创建工作簿对象
        XSSFSheet sheet = workbook.createSheet("学员名单");                     // 创建工作表
