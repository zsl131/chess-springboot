 ...
 */
@Component
public class ExportStudentRecordCountTools {
    ...
    static {
        columnName = new String [] {"姓名", "联系电话", "报名次数", "通过次数", "签到次数", "签到率"};
    }
...
//            System.out.println("========="+methodName);
    ...
    public void exportStudent(List<StudentCountDto> data, OutputStream os) {
        String title = "学生报名统计-"+ NormalTools.curDate("yyyyMMdd")+"-（"+data.size()+"人）";
        XSSFWorkbook workbook = new XSSFWorkbook();                        // 创建工作簿对象
        XSSFSheet sheet = workbook.createSheet("统计表");                     // 创建工作表
        ...
        //将查询出的数据设置到sheet对应的单元格中
        for (int i = 0; i < data.size(); i++) {
            StudentCountDto dto = data.get(i);//遍历每个对象
            XSSFRow row = sheet.createRow(i + 3);//创建所需的行数

            for (int j = 0; j < columnName.length; j++) {
                XSSFCell cell = row.createCell(j, XSSFCell.CELL_TYPE_STRING);//设置单元格的数据类型
                //第一列为数字类型并设置单元格的值
                if (j == 0) { cell.setCellValue(dto.getName()); }
                else if(j==1) {cell.setCellValue(dto.getPhone());}
                else if(j==2) {cell.setCellValue(dto.getTotal());}
                else if(j==3) {cell.setCellValue(dto.getPassCount());}
                else if(j==4) {cell.setCellValue(dto.getSignCount());}
                else if(j==5) {cell.setCellValue(dto.getSignPercent());}
                cell.setCellStyle(style); //设置单元格样式
            }
        }
