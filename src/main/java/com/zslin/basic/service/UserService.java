...
//            return JsonResult.succ(loginDto);
    ...
    /**
     * 通过用户名登陆，用于短信登陆和微信扫码登陆
     * @param params {username: 'root', token:'123'}
     * @return
     */
    public JsonResult loginByUsername(String params) {
        try {
            String username = JsonTools.getJsonParam(params, "username");
            String token = JsonTools.getJsonParam(params, "token");
            OtherLogin loginObj = otherLoginDao.findByUsernameAndToken(username, token);
            if(loginObj==null) {return JsonResult.error("登陆异常");}
